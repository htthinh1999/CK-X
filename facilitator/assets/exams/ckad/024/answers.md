...
## Question 1 | ConfigMap from file + label selector

> Server: `ssh ckad9988`

```bash
kubectl -n tides create configmap gauge-config \
  --from-file=station.conf=/home/candidate/exam/q1/gauge.properties
...
kubectl -n tides get configmaps -l 'tier=gauge,status!=retired' -o name \
  | cut -d/ -f2 | sort > /home/candidate/exam/q1/active-gauges.txt
cat /home/candidate/exam/q1/active-gauges.txt
# gauge-config
# tide-mouth
# tide-north
# tide-south
...
## Question 2 | Roll back a broken Deployment
...
kubectl -n quayside get deployment crane \
  -o jsonpath='{.metadata.annotations.deployment\.kubernetes\.io/revision}' > /home/candidate/exam/q2/revision.txt
cat /home/candidate/exam/q2/revision.txt   # 4
```
...
## Question 3 | Secret as volume and env var
...
## Question 4 | ClusterIP Service smoke test and Pod logs
...
mkdir -p /home/candidate/exam/q4
kubectl -n signal run fetch --image=busybox:1.36 --restart=Never --rm -i \
  -- wget -qO- http://foghorn-svc:8080/ > /home/candidate/exam/q4/response.html
kubectl -n signal logs foghorn > /home/candidate/exam/q4/foghorn.log
grep -i 'welcome to nginx' /home/candidate/exam/q4/response.html
grep 'GET /' /home/candidate/exam/q4/foghorn.log
```
...
## Question 5 | Pod with env and a jsonpath status command
...
echo "kubectl get pod lamp-keeper -n beacon -o jsonpath='{.status.phase}'" > /home/candidate/exam/q5/phase.sh
bash /home/candidate/exam/q5/phase.sh; echo     # Running
...
## Question 6 | ConfigMap change, rollout restart and annotation
...
## Question 7 | CronJob with non-root securityContext
...
## Question 8 | Kustomize canary overlay

> Server: `ssh ckad9999`

```bash
mkdir -p /home/candidate/exam/q8/overlays/canary
cat > /home/candidate/exam/q8/overlays/canary/kustomization.yaml <<'EOF'
...
patches:
  - patch: |-
      ...
      metadata:
        name: weighbridge
EOF
kubectl kustomize /home/candidate/exam/q8/overlays/canary      # only Deployment weighbridge-canary
kubectl apply -k /home/candidate/exam/q8/base
kubectl apply -k /home/candidate/exam/q8/overlays/canary
...
## Question 9 | HPA manifest migration to autoscaling/v2
...
cat > /home/candidate/exam/q9/forklift-hpa.yaml <<'EOF'
...
EOF
kubectl apply -f /home/candidate/exam/q9/forklift-hpa.yaml
...
## Question 10 | Manage Helm releases
...
helm -n berth upgrade tugboat /home/candidate/exam/q10/dockyard --reuse-values \
  --set replicaCount=2 --set podAnnotations.crew=night-shift

helm -n berth uninstall pilot-legacy

helm -n berth install harbormaster /home/candidate/exam/q10/dockyard --set service.type=NodePort
...
## Question 11 | Static hostPath PersistentVolume and claim
...
## Question 12 | Fix a misconfigured Service
...
## Question 13 | NetworkPolicy: database reachable only from the backend
...
## Question 14 | Exec readiness probe
...
## Question 15 | Logging sidecar for a file-based app
...
## Question 16 | LimitRange defaults and ResourceQuota
...
## Question 17 | Bulk labels and annotations with selectors
