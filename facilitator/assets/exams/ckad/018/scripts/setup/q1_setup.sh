...
mkdir -p /tmp/exam/course/1/base
cat > /tmp/exam/course/1/base/kustomization.yaml <<'EOF'
resources:
  - deployment.yaml
EOF
cat > /tmp/exam/course/1/base/deployment.yaml <<'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: app-deploy
...
cat > /tmp/exam/course/1/kustomization.yaml <<'EOF'
