...
# Extract the pod template and create /tmp/exam/course/4/pod.yaml
```

```yaml
# /tmp/exam/course/4/pod.yaml
apiVersion: v1
kind: Pod
metadata:
  name: tempest-debug
...
**Explanation:** Extract the template from the deployment and wrap it in a Pod definition, changing only the container command to `['sleep', '3600']`. Save the manifest at `/tmp/exam/course/4/pod.yaml`.
...
**Explanation:** Create `kustomization.yaml` at `/tmp/exam/course/8/` defining the resource and `configMapGenerator`, then apply it to the `tornado` namespace with `kubectl apply -k`.
...
kubectl top pods -n sirocco --sort-by=memory --no-headers | head -n 3 | awk '{print $1}' > /tmp/exam/course/10/top-pods.txt
```

**Explanation:** Use `kubectl top pods --sort-by=memory` to find the pods consuming the most memory and write the top 3 names, one per line, to `/tmp/exam/course/10/top-pods.txt`.
...
spec:
  ...
  - name: storage
    image: nginx:1.23.1
EOF
kubectl apply -f /tmp/exam/q14.yaml
```
...
spec:
  ...
  - type: Container
    ...
    defaultRequest:
      cpu: "200m"
EOF
kubectl apply -f /tmp/exam/q15.yaml
```
...
kubectl replace --force -f /tmp/exam/q16.yaml
```
...
spec:
  ...
  - ports:
    ...
    - protocol: TCP
      port: 53
EOF
kubectl apply -f /tmp/exam/q17.yaml
```
        ...
        backend:
          service:
            ...
            port:
              number: 80
EOF
kubectl apply -f /tmp/exam/q18.yaml
```
