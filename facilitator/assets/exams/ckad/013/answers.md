...
## Question 1 | Deploy with Kustomize (6 points)

> Server: `ssh ckad9988`

Create `/tmp/exam/course/1/kustomization.yaml`:
...
```bash
kubectl kustomize /tmp/exam/course/1/
kubectl apply -k /tmp/exam/course/1/
...
## Question 2 | ServiceAccount with RBAC and Verification (6 points)
...
kubectl auth can-i list deployments \
  ...
  > /tmp/exam/course/2/auth-check.txt

cat /tmp/exam/course/2/auth-check.txt
```

---

## Question 3 | Build Container Image and Save as Tarball (6 points)

> Server: `ssh ckad9999`

```bash
# Build the image
docker build -t solar-app:1.0 /tmp/exam/course/3/image/

# Verify
docker images solar-app:1.0

# Save as tarball
docker save -o /tmp/exam/course/3/solar-app.tar solar-app:1.0

# Verify
ls -lh /tmp/exam/course/3/solar-app.tar
```

---

## Question 4 | Helm Upgrade with Custom Values (4 points)
...
## Question 5 | Create Deployment with Labels and Annotations (4 points)
...
## Question 6 | ConfigMap from env-file (4 points)

> Server: `ssh ckad9977`

```bash
kubectl create configmap app-config --from-env-file=/tmp/exam/course/6/app.env -n eclipse
...
## Question 7 | Sidecar Container with Shared Volume (6 points)
...
## Question 8 | Create docker-registry Secret (4 points)
...
## Question 10 | NetworkPolicy with ipBlock (6 points)
...
## Question 12 | Ingress with TLS Termination (6 points)
...
## Question 13 | Discover and Use Custom Resource Definition (6 points)
...
echo "ckad.example.com" > /tmp/exam/course/13/crd-group.txt
...
## Question 14 | Create PVC and Mount in Pod (6 points)
...
## Question 15 | Create TLS Secret (4 points)

> Server: `ssh ckad9988`

```bash
kubectl create secret tls web-tls \
  --cert=/tmp/exam/course/15/tls.crt \
  --key=/tmp/exam/course/15/tls.key \
  -n flare
...
## Question 16 | Blue/Green Deployment (8 points)
...
## Question 17 | Fix Service and Verify DNS Resolution (4 points)
...
kubectl run tmp-dns --rm -i --restart=Never --image=busybox:1.36 -n sunbeam \
  -- nslookup dns-svc.sunbeam.svc.cluster.local > /tmp/exam/course/17/dns-output.txt

cat /tmp/exam/course/17/dns-output.txt
```

---

## Question 18 | Configure Rolling Update Strategy (6 points)
...
## Question 19 | Harden Deployment with SecurityContext (4 points)
...
## Question 20 | Troubleshoot Pending Pod (6 points)
...
# Save the reason
echo "disktype" > /tmp/exam/course/20/pending-reason.txt
