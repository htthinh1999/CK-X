...
# helm template phoenix-web bitnami/nginx -n flare -f /tmp/phoenix-values.yaml > /tmp/exam/course/4/rendered.yaml
```
...
kubectl create secret generic db-credentials \
  --from-file=password.txt=/tmp/exam/course/7/password.txt \
  -n magma
```
...
helm install phoenix-api bitnami/nginx \
  -n flare \
  -f /tmp/exam/course/13/values.yaml
```
