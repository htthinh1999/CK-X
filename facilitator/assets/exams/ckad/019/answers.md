...
## Question 2 | Kustomize Image + Replica Patch

> Server: `ssh ckad9988`

```yaml
# /tmp/exam/course/2/kustomization.yaml
...
```bash
kubectl kustomize /tmp/exam/course/2/ > /tmp/exam/course/2/kustomize-output.yaml
kubectl apply -f /tmp/exam/course/2/kustomize-output.yaml -n vanguard
```

---

## Question 3 | Selective Secret Key Volume

> Server: `ssh ckad9977`

```yaml
# /tmp/exam/course/3/multi-secret.yaml
apiVersion: v1
kind: Secret
metadata:
  name: db-credentials
...
```bash
kubectl apply -f /tmp/exam/course/3/multi-secret.yaml
```
...
## Question 4 | ConfigMap as Init Script Volume
...
```yaml
# /tmp/exam/course/4/init-pod.yaml
apiVersion: v1
kind: Pod
metadata:
  name: web-setup
...
```bash
kubectl apply -f /tmp/exam/course/4/init-pod.yaml
```

---

## Question 5 | LimitRange Defaults

> Server: `ssh ckad9977`

```yaml
# /tmp/exam/course/5/limit-range.yaml
apiVersion: v1
kind: LimitRange
metadata:
  name: rampart-limits
...
```bash
kubectl apply -f /tmp/exam/course/5/limit-range.yaml
```
...
## Question 6 | CronJob Scheduled Report
...
kubectl create cronjob siege-report -n siege --image=busybox:1.36 \
  ...
  -- /bin/sh -c "date; echo Hello from siege" > /tmp/exam/course/6/cronjob.yaml
...
## Question 7 | Ingress NetworkPolicy

> Server: `ssh ckad9977`

```yaml
# /tmp/exam/course/7/netpol.yaml
...
```bash
kubectl apply -f /tmp/exam/course/7/netpol.yaml
```

---

## Question 8 | Sidecar Process Monitor

> Server: `ssh ckad9999`

```yaml
# /tmp/exam/course/8/shared-pid.yaml
apiVersion: v1
kind: Pod
metadata:
...
```bash
kubectl apply -f /tmp/exam/course/8/shared-pid.yaml
```
...
## Question 10 | Helm Release Upgrade (Atomic)

> Server: `ssh ckad9999`

```bash
helm upgrade battle-web /tmp/exam/course/10/battle-chart/ -n garrison \
...
## Question 11 | Ephemeral Debug Container
...
## Question 12 | Deployment with progressDeadlineSeconds

> Server: `ssh ckad9999`

```bash
kubectl create deployment citadel-guard -n citadel --image=nginx:1.24.0-alpine \
  --replicas=4 --dry-run=client -o yaml > /tmp/exam/course/12/deploy.yaml
...
```bash
kubectl apply -f /tmp/exam/course/12/deploy.yaml
```

---

## Question 13 | ResourceQuota Troubleshooting
...
## Question 14 | Canary Ingress

> Server: `ssh ckad9977`

```yaml
# /tmp/exam/course/14/ingress.yaml
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: canary-ingress
...
```bash
kubectl apply -f /tmp/exam/course/14/ingress.yaml
```

---

## Question 15 | Downward API Env Vars

> Server: `ssh ckad9988`

```yaml
# /tmp/exam/course/15/downward.yaml
apiVersion: v1
kind: Pod
metadata:
  name: resource-aware
...
```bash
kubectl apply -f /tmp/exam/course/15/downward.yaml
```

---

## Question 16 | Selectorless Service + Endpoints

> Server: `ssh ckad9977`

```yaml
# /tmp/exam/course/16/manual-svc.yaml
apiVersion: v1
kind: Service
metadata:
  name: external-db
...
```bash
kubectl apply -f /tmp/exam/course/16/manual-svc.yaml
```
...
## Question 17 | Blue-Green Service Switch
...
## Question 18 | ServiceAccount with Manual Token Mount

> Server: `ssh ckad9988`

```yaml
# /tmp/exam/course/18/sa-pod.yaml
apiVersion: v1
kind: ServiceAccount
metadata:
  name: stealth-sa
...
```bash
kubectl apply -f /tmp/exam/course/18/sa-pod.yaml
```

---

## Question 19 | CoreDNS Rewrite Rule

> Server: `ssh ckad9977`

```bash
kubectl get configmap coredns -n kube-system -o yaml > /tmp/exam/course/19/coredns.yaml
...
kubectl apply -f /tmp/exam/course/19/coredns.yaml
```
...
The saved `/tmp/exam/course/19/coredns.yaml` contains the `rewrite name exact hachiman.local hachiman.garrison.svc.cluster.local` line. No CoreDNS pod restart is required.

---

## Question 20 | SecurityContext + seccomp

> Server: `ssh ckad9988`

```yaml
# /tmp/exam/course/20/seccomp.yaml
apiVersion: v1
kind: Pod
metadata:
...
```bash
kubectl apply -f /tmp/exam/course/20/seccomp.yaml
```
