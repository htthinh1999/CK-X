...
## Question 1 | Kustomize ConfigMap Generator

> Server: `ssh ckad9988`

```bash
# Modify kustomization.yaml
cat <<EOF >> /tmp/exam/course/1/kustomize/kustomization.yaml
secretGenerator:
- name: matrix-secret
  literals:
...
kubectl kustomize /tmp/exam/course/1/kustomize | kubectl apply -n matrix -f -
```
...
## Question 2 | ClusterRole and ClusterRoleBinding
...
## Question 3 | Init Container Failure Debug
...
## Question 4 | ResourceQuota for Namespace
...
## Question 5 | Namespace Events Export

> Server: `ssh ckad9988`

```bash
kubectl get events -n zenith -o custom-columns=TYPE:.type,REASON:.reason,MESSAGE:.message > /tmp/exam/course/5/events.txt
```
...
## Question 6 | NetworkPolicy Named Port
...
## Question 7 | Service Internal Traffic Policy

> Server: `ssh ckad9988`

```bash
cat <<EOF > /tmp/exam/course/7/check.sh
#!/bin/bash
kubectl port-forward svc/backend-api 9999:8080 -n genesis &
PF_PID=\$!
sleep 2
curl http://localhost:9999/health >> /tmp/exam/course/7/health.log
kill \$PF_PID
EOF
chmod +x /tmp/exam/course/7/check.sh
```
...
## Question 8 | Multi-Stage Dockerfile Build

> Server: `ssh ckad9999`

```bash
# 1. Modify Dockerfile
cat <<EOF > /tmp/exam/course/8/Dockerfile
FROM nginx:1.21
...
# 2. Build image
docker build -t localhost:5000/genesis-app:v1 /tmp/exam/course/8/
...
## Question 9 | Projected ServiceAccount Token
...
## Question 10 | Adapter Pattern Sidecar
...
## Question 11 | Ingress Default Backend
...
## Question 12 | Parallel Job with Completions
...
## Question 13 | Service Endpoint Inspection

> Server: `ssh ckad9977`

```bash
kubectl run dns-tester -n zenith --image=busybox:1.32 -- sleep 3600
echo "data-svc.ancient.svc.cluster.local" > /tmp/exam/course/13/fqdn.txt
```
...
## Question 14 | Pod Lifecycle PreStop Hook
...
## Question 15 | Namespace Isolation NetworkPolicy
...
## Question 16 | Helm Release Values Override
...
helm upgrade genesis-web /tmp/exam/course/16/genesis-web-chart -n nexus --reuse-values --set replicaCount=3
...
## Question 17 | Secret stringData Entry
...
## Question 18 | Deployment Canary Split
...
## Question 19 | Pod with ReadOnlyRootFilesystem
...
## Question 20 | Deployment Rollback
