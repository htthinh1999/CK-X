...
## Question 1 | Service ExternalName
...
kubectl explain pod.spec.containers.resources --recursive > /tmp/exam/course/2/pod-spec-fields.txt
```
...
## Question 6 | Pod Security Context
...
## Question 8 | Container Capabilities
...
## Question 9 | Deployment Rollout Control
...
kubectl top pods -n jungle > /tmp/exam/course/14/pod-resources.txt 2>&1

# Top CPU consumer
kubectl top pods -n jungle --sort-by=cpu 2>/dev/null | head -2 | tail -1 | awk '{print $1}' > /tmp/exam/course/14/top-cpu-pod.txt

# Fallback if metrics-server is not installed
if ! kubectl top pods -n jungle >/dev/null 2>&1; then
  echo "Metrics server not available" > /tmp/exam/course/14/pod-resources.txt
  echo "unknown" > /tmp/exam/course/14/top-cpu-pod.txt
fi
```
...
## Question 15 | Deployment Safe Rollout
...
## Question 17 | Container Lifecycle Hook
...
## Question 18 | PodDisruptionBudget
...
## Question 19 | NetworkPolicy Default Deny
