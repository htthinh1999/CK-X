...
docker save -o /tmp/exam/course/5/my-app.tar my-app:1.0

# Verify
ls -lh /tmp/exam/course/5/my-app.tar
```
...
```bash
# Apply and verify
kubectl apply -f /tmp/exam/course/8/broken-deploy.yaml
...
kubectl rollout history deploy app-v1 -n brook | tail -1 | awk '{print $1}' > /tmp/exam/course/9/rollback-revision.txt
```
