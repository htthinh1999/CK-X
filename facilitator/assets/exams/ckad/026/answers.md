...
## Question 2 | Blue/green cut-over with a named target port
...
kubectl -n junction get svc junction -o json \
  ...
  > /home/candidate/exam/q2/old-selector.txt
cat /home/candidate/exam/q2/old-selector.txt        # app=junction,slot=blue,tier=web
...
## Question 3 | Helm forensics: repair a failed release, upgrade keeping values
...
echo stop-indexer > /home/candidate/exam/q3/failed-release.txt

# a failed release can be upgraded in place; keep its values, change only the tag
helm -n helmyard get values stop-indexer
helm -n helmyard upgrade stop-indexer /home/candidate/exam/q3/depot-web-0.1.0.tgz \
...
helm -n helmyard get values route-planner -o yaml > /home/candidate/exam/q3/route-planner-values.yaml
helm -n helmyard upgrade route-planner /home/candidate/exam/q3/depot-web-0.2.0.tgz \
  -f /home/candidate/exam/q3/route-planner-values.yaml --wait --timeout 120s
...
## Question 4 | Pending Pods on a tainted node
...
echo "$NODE" > /home/candidate/exam/q4/depot-node.txt
```
...
## Question 5 | startupProbe for a slow-starting container
...
echo sleeper-berths > /home/candidate/exam/q5/deployment.txt
...
## Question 6 | CreateContainerConfigError from missing ConfigMap keys
...
echo CreateContainerConfigError > /home/candidate/exam/q6/reason.txt

grep -E '^(claim\.window|ledger\.path)=' /home/candidate/exam/q6/desk-settings.properties
...
## Question 7 | ConfigMap updates that reach running Pods (no subPath)
...
## Question 8 | Init container rendering config via the downward API
...
cat > /home/candidate/exam/q8/signal-box.yaml <<'YAML'
apiVersion: v1
kind: Pod
metadata:
  name: signal-box
...
spec:
  ...
  - name: config
    emptyDir: {}
YAML
kubectl apply -f /home/candidate/exam/q8/signal-box.yaml
...
## Question 10 | Pod Security Admission: make a Deployment restricted-compliant
...
cat > /home/candidate/exam/q10/patch.json <<'EOF'
[
...
EOF
kubectl -n yardsafety patch deployment shunter --type=json --patch-file /home/candidate/exam/q10/patch.json
...
## Question 11 | Pull secrets via the ServiceAccount and a TLS Secret
...
cd /home/candidate/exam/q11/certs
...
## Question 12 | Build with build arguments, push to the local registry, deploy

> Server: `ssh ckad9999`

```bash
cd /home/candidate/exam/q12/app
cat Dockerfile
...
docker image inspect localhost:5000/dispatch-board:2.3 \
  --format '{{ index .Config.Labels "transit.dispatch/release" }}' > /home/candidate/exam/q12/release.txt
cat /home/candidate/exam/q12/release.txt          # riverside-417
...
## Question 13 | Pending Pods: find the unsatisfiable request
...
echo cache:memory > /home/candidate/exam/q13/blocker.txt
...
## Question 14 | Ingress with Traefik, named Service ports and a decoy Service
...
kubectl -n platform run q5-departures --image=busybox:1.36 --restart=Never --rm -i -- \
  ...
  > /home/candidate/exam/q14/departures.out
kubectl -n platform run q5-arrivals --image=busybox:1.36 --restart=Never --rm -i -- \
  ...
  > /home/candidate/exam/q14/arrivals.out
cat /home/candidate/exam/q14/departures.out /home/candidate/exam/q14/arrivals.out
```
...
## Question 15 | Custom resources with a clashing resource name
...
kubectl -n manifests get routes.transit.example.com --sort-by=.spec.stops --no-headers \
  ...
  > /home/candidate/exam/q15/routes.txt
cat /home/candidate/exam/q15/routes.txt
```
...
## Question 16 | Kustomize prod overlay: selectors and generator merge

> Server: `ssh ckad9988`

```bash
mkdir -p /home/candidate/exam/q16/overlays/prod
cat > /home/candidate/exam/q16/overlays/prod/kustomization.yaml <<'EOF2'
...
EOF2
kubectl kustomize /home/candidate/exam/q16/overlays/prod
kubectl apply -k /home/candidate/exam/q16/overlays/prod
...
## Question 17 | NetworkPolicy: namespace AND pod selector
...
## Question 18 | Least-privilege ServiceAccount with resourceNames
...
} > /home/candidate/exam/q18/can-i.txt
cat /home/candidate/exam/q18/can-i.txt            # yes / yes / no
```
