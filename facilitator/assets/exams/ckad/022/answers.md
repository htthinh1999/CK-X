...
kubectl run musashi-pod --image=localhost:5000/musashi-app:v1 -n apex --dry-run=client -o yaml > /tmp/exam/course/1/pod.yaml
...
cat <<EOF > /tmp/exam/course/2/multi-pod.yaml
apiVersion: v1
kind: Pod
metadata:
  name: tri-blade
...
EOF
kubectl apply -f /tmp/exam/course/2/multi-pod.yaml
```
...
spec:
  ...
  template:
    spec:
      ...
      restartPolicy: Never
EOF
kubectl apply -f /tmp/exam/course/3/job.yaml
```
...
cat <<EOF > /tmp/exam/course/4/cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: db-backup
...
EOF
kubectl apply -f /tmp/exam/course/4/cronjob.yaml
```
...
image:
  ...
  tag: alpine
EOF
helm install crown-release /tmp/exam/course/5/my-chart -n crown -f /tmp/exam/course/5/values.yaml
```
...
cat <<EOF > /tmp/exam/course/7/canary.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: legacy-canary
  ...
  template:
    ...
    spec:
      ...
      - name: web
        image: httpd:2.4
EOF
kubectl apply -f /tmp/exam/course/7/canary.yaml
...
- patch: |-
    ...
    spec:
      replicas: 4
EOF
kubectl apply -k /tmp/exam/course/8/prod -n mastery
```
...
Filters the pod logs for `ERROR` lines into `/tmp/exam/course/10/logs.txt`.
...
cat <<EOF > /tmp/exam/course/12/secure-pod.yaml
apiVersion: v1
kind: Pod
metadata:
  name: secure-pod
...
EOF
kubectl apply -f /tmp/exam/course/12/secure-pod.yaml
```
...
cat <<EOF > /tmp/exam/course/14/inject.yaml
apiVersion: v1
kind: Pod
metadata:
  name: inject-pod
...
EOF
kubectl apply -f /tmp/exam/course/14/inject.yaml
```
...
cat <<EOF > /tmp/exam/course/15/quota.yaml
apiVersion: v1
kind: LimitRange
metadata:
  name: cpu-limits
...
spec:
  hard:
    ...
    limits.memory: 4Gi
EOF
kubectl apply -f /tmp/exam/course/15/quota.yaml
```
...
cat <<EOF > /tmp/exam/course/16/storage.yaml
apiVersion: v1
...
spec:
  ...
  - name: data
    persistentVolumeClaim:
      claimName: glory-pvc
EOF
kubectl apply -f /tmp/exam/course/16/storage.yaml
```
...
cat <<EOF > /tmp/exam/course/20/dns.yaml
apiVersion: v1
kind: Pod
metadata:
  name: dns-test
...
spec:
  ...
  - name: main
    ...
    command: ["sleep", "3600"]
EOF
kubectl apply -f /tmp/exam/course/20/dns.yaml
...
kubectl exec dns-test -n triumph -- nslookup -type=srv _https._tcp.kubernetes.default.svc.cluster.local > /tmp/exam/course/20/dns-output.txt
```

Creates pod `dns-test` and saves the SRV lookup for the `kubernetes.default` service to `/tmp/exam/course/20/dns-output.txt`.
