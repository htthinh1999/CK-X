...
## Question 1: Create a CRD (CustomResourceDefinition) for a new resource type 'Backup' in API group 'data.example.com' with version 'v1alpha1' that includes fields 'spec.source' and 'spec.destination'
...
## Question 2: Pod 'logging-pod' in namespace 'troubleshooting' is consuming excessive CPU resources. Set appropriate CPU and memory limits
...
## Question 3: Create a NetworkPolicy named 'allow-traffic' in namespace 'networking' that allows traffic to pods with label 'app=web' only from pods with label 'tier=frontend' on port 80
...
## Question 4: Create a ConfigMap named 'app-config' in namespace 'workloads' containing the following key-value pairs: APP_ENV=production, LOG_LEVEL=info. Then create a Pod named 'config-pod' using 'nginx' image that mounts these configurations as environment variables
...
## Question 5: Create a deployment called nginx-deployment in the namespace dev with 3 replicas and image nginx:latest
...
## Question 6: Create a ClusterIP service named 'internal-app' in namespace 'networking' that routes traffic to pods with label 'app=backend' on port 8080, exposing the service on port 80
...
## Question 7: Create a PersistentVolume named 'pv-storage' with 1Gi capacity, access mode ReadWriteOnce, hostPath type at /mnt/data, and reclaim policy Retain
...
## Question 8: Create a NodePort service named public-web in namespace networking that will expose the web-frontend deployment to external users.
...
## Question 9: Create a StorageClass named 'fast-storage' with provisioner 'kubernetes.io/no-provisioner' and volumeBindingMode 'WaitForFirstConsumer'
...
## Question 11: Create an Ingress resource named 'api-ingress' in namespace 'networking' that routes traffic from 'api.example.com' to the service 'api-service' on port 80
...
## Question 12: Create a CronJob named 'log-cleaner' in namespace 'workloads' that runs hourly to clean up log files
...
## Question 13: Create a PersistentVolumeClaim named 'pvc-app' that requests 500Mi of storage with ReadWriteOnce access mode and uses the 'fast-storage' StorageClass
...
## Question 14: Create a Pod named 'health-pod' in namespace 'workloads' using 'nginx' image with a liveness probe that checks the path /healthz on port 80 every 15 seconds, and a readiness probe that checks port 80 every 10 seconds
...
## Question 15: The deployment 'broken-app' in namespace 'troubleshooting' is failing to start. Identify and fix the issue
...
## Question 16: Create a ClusterRole named 'pod-reader' that allows getting, watching, and listing pods. Then create a ClusterRoleBinding named 'read-pods' that grants this role to the user 'jane' in the namespace 'cluster-admin'
...
## Question 17: Create a multi-container pod with sidecar logging pattern
...
## Question 18: Create a simple Kubernetes Job named 'hello-job' that executes a command and completes
...
## Question 19: Deploy the Bitnami Nginx chart in the 'web' namespace using Helm
...
## Question 20: Work with the Open Container Initiative (OCI) format
...
## Question 21: Service 'web-service' in namespace 'troubleshooting' is not routing traffic to pods properly. Identify and fix the issue
