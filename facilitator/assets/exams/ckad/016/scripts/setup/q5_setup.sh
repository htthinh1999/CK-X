...
cat > /tmp/exam/course/5/chart/Chart.yaml <<'YAML'
apiVersion: v2
name: thunder-web
...
cat > /tmp/exam/course/5/chart/values.yaml <<'YAML'
replicaCount: 1
image:
  repository: nginx
...
cat > /tmp/exam/course/5/chart/templates/deployment.yaml <<'YAML'
apiVersion: apps/v1
kind: Deployment
metadata:
...
cat > /tmp/exam/course/5/chart/templates/_helpers.tpl <<'YAML'
...
rm -f /tmp/exam/course/5/output.yaml 2>/dev/null || true
